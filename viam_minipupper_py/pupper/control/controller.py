...
from viam_minipupper_py.pupper.control.gait import GaitController
    ...
    def run(self, state, command, location, attitude, robot_speed):
        ...
        if state.behavior_state == BehaviorState.TROT:
            ...
            state.foot_locations = (
                self.config.default_stance + np.array([0, 0, -0.03])[:, np.newaxis]
            )
            state.joint_angles = self.inverse_kinematics(
            ...
            state.foot_locations = (
                self.config.default_stance + np.array([0, 0, -0.105])[:, np.newaxis]
            )
            state.joint_angles = self.inverse_kinematics(
                state.foot_locations, self.config
            )
