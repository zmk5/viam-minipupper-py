...
class Movements:
    def __init__(
        self, name, speed_enable, attitude_enable, legs_enable, actuator_enable
    ):
...
class MovementScheme:
    ...
    def updateMovement(self, movement_type):
        ...
        if self.ststus == "Entry":
            ...
            self.legs_location_now, self.entry_down = self.updateMovementGradient(
...
def appendDanceMovement() -> List[Movements]:
    ...
    # demo 2
    dance_scheme = Movements(
        "push-up", "SpeedEnable", "AttitudeDisable", "LegsEnable", "ActuatorDisable"
    )
    dance_scheme.setExitstate("Stand")
